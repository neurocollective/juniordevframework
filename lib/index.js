...
// TODO - enforce function signature as taking the whole object,
// to remain consistent w/ other functions
const isTokenExpiredByAPICheck = async (token = '') => {
  let accessTokenValue = token;
...
const buildScrubberUtility = (dbRows) => {
  ...
  const scrubResultsForJobTitle = (searchedJobTitle, results) => {
    const accum = { existingListingsMap: getExistingListingsMap(), newResultObjects: [] };
    ...
    // TODO - don't assume they are all new, only add
    // the entries that are NOT in existingListingsMap
...
const wipeHTMLTagsFromString = (stringThatMaybeHasHTML) => {
  ...
  const { wipedString: result } = rangesToWipe.reduce((accumulator, twoIndexRangeList, index) => {
    ...
    if (index === rangesCount - 1) {
      portionToRetain += originalString.slice(twoIndexRangeList[1] + 1, originalString.length);
    }
...
const requestToken = async (credentials, code) => {
  ...
  try {
    ...
    const TOKEN_FILE_PATH = `${__dirname}/../server/token.json`;
...
const refreshToken = async (credentials, existingTokenObject, userId) => {
  if (!credentials || !existingTokenObject || !userId) {
    throw new Error('credentials, existingTokenObject, and userId required for refreshToken');
  }
  ...
  try {
    ...
    console.error('refreshToken got an error:', error.response.body);
    return null;
  }
};
...
const getGmailProfile = async (token = '') => {
  ...
  try {
    ...
    if (statusCode === 403) {
      // Gmail API has not been used in project xxxx before or it is disabled.
      // Enable it by visiting yyyyyy then retry. If you enabled this API recently,
...
// TODO - remove default value for `dateString`
const listGmailMessages = async (tokenObject = {}, dateString = '2020/01/01') => {
  const { access_token: token } = tokenObject;

  // const uri = 'https://gmail.googleapis.com/gmail/v1/users/me/messages?fields=internalDate';
  const query = encodeURIComponent(`after:${dateString}`);
  const uri = `https://gmail.googleapis.com/gmail/v1/users/me/messages?${query}`;
...
const getGmailMessageById = async (tokenObject, id = '') => {
  const { access_token: token } = tokenObject;

  const uri = `https://gmail.googleapis.com/gmail/v1/users/me/messages/${id}`;
...
const decodeBase64String = (base64Encoded) => {
  const buffer = Buffer.from(base64Encoded, 'base64');
  return buffer.toString();
};

const convertEpochValueToGmailDate = (unixEpoch) => {
  const asMoment = moment(unixEpoch);
  const year = asMoment.year();
  const month = asMoment.month() + 1;
