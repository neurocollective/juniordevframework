...
const isTokenExpiredByAPICheck = async (token) => {
  ...
  try {
    const { error, response, statusCode } = await getGmailProfile(token['access_token']);
  } catch (err) {
...
const requestToken = async (credentials, code) => {
  ...
  try {
    ...
    // await writeStringToFile(TOKEN_FILE_PATH, JSON.stringify(body));

    return { response: JSON.parse(body), statusCode };
  } catch (error) {
...
const getGmailProfile = async (token) => {
  ...
  try {
    ...
    return { response: JSON.parse(body), statusCode: status, error: null };
  } catch (error) {
