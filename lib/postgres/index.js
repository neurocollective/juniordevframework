...
const buildCreateNewUser = (client) => async (email, firstName, lastName, tokenObject) => {
  await client.query('BEGIN;');
  let userId;
  try {
    const insertQuery = queryBuilders.buildInsertUserQuery(email, firstName, lastName);
    ({ rows: [{ id: userId  }] } = await client.query(insertQuery));

    // need to insert last scan date, now that there is a suser
    const now = moment();
    const dateString = now.format('MM/DD/YYYY');

    const query = buildInsertEmailScanRecordQuery(userId, dateString, now.unix());
  ...
  return {
    ...
    insertSeedDataIntoTables: async (credentialsObject, tokenObject, gmail) => {
      ...
      if (tokenObject && gmailObjectValid) {
        ...
        const createNewUser = buildCreateNewUser(client);
        await createNewUser(email, firstName, lastName, tokenObject);

        // const insertQuery = queryBuilders.buildInsertUserQuery(email, firstName, lastName);
        // const { rows: [{ id: userId  }] } = await client.query(insertQuery);
    ...
    createNewUser: buildCreateNewUser(client),
    ...
    insertNewUser: (email, firstName, lastName, hashedPassword) => {
      console.log('PLEASE NOTE: `insertNewUser` is deprecated, please use `createNewUser`');
    ...
    insertEmailScanRecord: (userId, dateString, unixEpoch) => {
      const query = buildInsertEmailScanRecordQuery(userId, dateString, unixEpoch);
      return client.query(query);
    },
