...
const createJobSearchContactTableQuery = `
  CREATE TABLE ${JOB_SEARCH_CONTACT} (
    ...
    name text,
    -- email text -- this will go in jobSearchContactEAV
  );
`;

const createJobSearchContactEAVTableQuery = `
  CREATE TABLE ${JOB_SEARCH_CONTACT}_eav (
    id serial PRIMARY KEY,
    contact_id integer REFERENCES ${JOB_SEARCH_ENTITY}(name)
    key text NOT NULL,
    value text NOT NULL
  );
`;
...
const createEmailScanRecordTableQuery = `
  CREATE TABLE ${EMAIL_SCAN_RECORD} (
    id serial PRIMARY KEY,
    user_id integer REFERENCES ${APP_USER}(id),
    last_scan timestamp, -- is this column needed?
    last_email_scan_date text,
    last_scan_epoch int,
    UNIQUE (user_id)
  );
`;
...
// TODO - this replaces EMAIL_SCAN_RECORD right?
// remove `createEmailScanRecordTableQuery` and functions
// const createLastEmailScanTableQuery = `
//   CREATE TABLE ${LAST_EMAIL_SCAN} (
//     id serial PRIMARY KEY,
//     user_id int REFERENCES app_user(id),
//     last_email_scan_date string NOT NULL,
//     last_scan_epoch int NOT NULL
//   );
// `;

const createEdgeDateEmailsTableQuery = `
  CREATE TABLE ${EDGE_DATE_EMAILS} (
    id serial PRIMARY KEY,
    user_id int REFERENCES app_user(id),
...
const createUnrecognizedEmailsTableQuery = `
...
export default {
  ...
  createUnrecognizedEmailsTableQuery,
  // createLastEmailScanTableQuery,
  createEdgeDateEmailsTableQuery,
};
