...
const createJobListingTableQuery = `
  CREATE TABLE ${JOB_LISTING} (
    ...
    entity_name int REFERENCES ${JOB_SEARCH_ENTITY}(id), -- allow as null for now
...
const createJobSearchContactTableQuery = `
  CREATE TABLE ${JOB_SEARCH_CONTACT} (
    ...
    name text,
    email text,
    entity_name int ${JOB_SEARCH_ENTITY}(id), -- allow as null for now
  );
`;
...
const createContactActionTableQuery = `
  CREATE TABLE ${CONTACT_ACTION} (
    id serial PRIMARY KEY,
    contact_id integer REFERENCES ${APP_USER}(id),
    ${ACTION_TYPE} text NOT NULL,
    ${COMMUNICATION_CHANNEL} string NOT NULL,
    action_date timestamp,
    action_notes text
    CHECK (
      ${ACTION_TYPE} = 'apply' OR ${ACTION_TYPE} = 'first-contact' OR ${ACTION_TYPE} = 'follow-up'
    )
    CHECK (
      ${COMMUNICATION_CHANNEL} = '${EMAIL}' OR ${COMMUNICATION_CHANNEL} = '${PHONE}' OR ${COMMUNICATION_CHANNEL} = '${IN_PERSON}'
      OR ${COMMUNICATION_CHANNEL} = '${TEXT}' OR ${COMMUNICATION_CHANNEL} = '${LINKEDIN}' OR ${COMMUNICATION_CHANNEL} = '${INTERNET}'
    )
  );
`;
